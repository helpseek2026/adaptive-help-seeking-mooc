...
# per_user_three_categories.csv is provided as a data file in this directory
# (derived from user-xiaomu.json according to the coding manual).
three_cat = pd.read_csv('per_user_three_categories.csv')

# NAMING / MERGE NOTES:
#  - Both tables contain a 'task_pct' column. After the merge below they become
#    'task_pct_orig' (four-level Task share, from the AI table) and
#    'task_pct_new' (three-category Task share, from per_user_three_categories).
#    The two are numerically near-identical; the three-category model below uses
#    'task_pct_new'.
#  - 'self_reg_pct' (AI table) and 'selfreg_pct' (three-category table) are the
#    SAME Self-Regulation variable under two spellings; both are carried through
...
# Save for later
merged_filt.to_csv(os.path.join(OUTPUT_DIR, 'multilevel_long_format.csv'), index=False)
...
results_df.to_csv(os.path.join(OUTPUT_DIR, 'multilevel_results_summary.csv'), index=False)
...
with open(os.path.join(OUTPUT_DIR, 'multilevel_key_stats.txt'), 'w') as f:
