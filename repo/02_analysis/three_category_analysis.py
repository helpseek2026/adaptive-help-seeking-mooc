...
# NOTE on naming: the column 'selfreg_pct' in this file is the SAME variable as
# 'self_reg_pct' in combined_ai_learning_data.csv (the four-level framework's
# Self-Regulation share: TA queries, FAQ, progress monitoring). The two names
# are identical in value; only the spelling differs across files.
...
corr_df.to_csv(os.path.join(OUTPUT_DIR, 'three_category_correlations.csv'), index=False)
...
# Fixed cutpoints matching the manuscript (Table 5) and OR3 §3.3
...
tert_summary.to_csv(os.path.join(OUTPUT_DIR, 'three_category_tertiles.csv'))
...
ttest_df.to_csv(os.path.join(OUTPUT_DIR, 'three_category_ttests.csv'), index=False)
...
for i, (bar, r, lo, hi) in enumerate(zip(bars, r_vals, ci_lo, ci_hi)):
    ...
    if r > 0:
        ...
        # 负值: 标签放在柱底下方,避免和0轴附近重叠
        label_y = r - max(yerr_lo, 0.005) - 0.003
        va = 'top'
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'Figure_three_category_analysis.png'), 
            dpi=300, bbox_inches='tight')
plt.savefig(os.path.join(OUTPUT_DIR, 'Figure_three_category_analysis.pdf'), 
            bbox_inches='tight')
...
1. KG Navigation accounts for an aggregate {total_kg/total_process*100:.1f}% of all process-level
   interactions, with a per-user mean share of {kg_share_clean.mean():.1f}% and a
   per-user median share of {kg_share_clean.median():.1f}%.

2. Across the five AI categories, usage proportions form a consistent GRADIENT
   with respect to accuracy:
   - KG Navigation:   r = {corr_results[0]['r (Pearson)']:+.4f}, p = {corr_results[0]['p (Pearson)']:.3f}  (negative)
   - Other Process:   r = {corr_results[1]['r (Pearson)']:+.4f}, p = {corr_results[1]['p (Pearson)']:.3f}  (ns)
   - Task-level:      r = {corr_results[2]['r (Pearson)']:+.4f}, p = {corr_results[2]['p (Pearson)']:.3f}  (positive)
   - Self-Regulation: r = {corr_results[3]['r (Pearson)']:+.4f}, p = {corr_results[3]['p (Pearson)']:.3f}  (positive)
   - Self-level:      r = {corr_results[4]['r (Pearson)']:+.4f}, p = {corr_results[4]['p (Pearson)']:.3f}  (positive)

   Process-oriented engagement is negatively associated with accuracy, whereas
   the more self-directed and task-oriented levels are positively associated.

3. The Low-vs-High tertile gap in KG usage ({gaps['kg_pct']:+.1f} pp, d = {ttest_results[0]["Cohen's d"]:+.3f})
   is the largest among the process-side features, while Task-level shows the
   inverse gap ({gaps['task_pct']:+.1f} pp, d = {ttest_results[2]["Cohen's d"]:+.3f}).

4. Within the process-level category, the aggregate -0.102 "effective feedback"
   correlation is driven by KG Navigation specifically rather than by
   process-level feedback as a whole.

INTERPRETATION:
The within-process aggregate is concentrated in KG Navigation. More broadly,
the cross-level gradient (process-side negative; task/self-side positive) is
consistent with performance-based differential selection across feedback
levels — lower performers engaging process-level features more heavily,
higher performers engaging self-regulatory and task-level features more —
rather than with a uniform feedback-effectiveness effect. This provides a more
precise framing of what the data support and aligns with Section 4.7.2.
""")
