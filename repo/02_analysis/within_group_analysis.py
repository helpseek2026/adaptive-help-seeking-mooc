...
# Step 1: Define performance tertiles using the same cutoffs as the original analysis
# (sensitivity report uses 67.4% and 84.0% as cutpoints)
df_sorted = df.sort_values('problem_accuracy').reset_index(drop=True)
n = len(df_sorted)
t1_cut = df_sorted['problem_accuracy'].iloc[n // 3]
t2_cut = df_sorted['problem_accuracy'].iloc[2 * n // 3]

print(f"\nTertile cutoffs:")
print(f"  Low (T1):    accuracy < {t1_cut:.2f}%")
print(f"  Medium (T2): {t1_cut:.2f}% <= accuracy < {t2_cut:.2f}%")
print(f"  High (T3):   accuracy >= {t2_cut:.2f}%")
...
# Save results
results_df.to_csv('/home/claude/analysis/within_group_correlations.csv', index=False)
...
plt.tight_layout()
plt.savefig('/home/claude/analysis/Figure_within_group_correlations.png', 
            dpi=300, bbox_inches='tight')
plt.savefig('/home/claude/analysis/Figure_within_group_correlations.pdf', 
            bbox_inches='tight')
